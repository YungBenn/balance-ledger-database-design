...
INSERT INTO ledger (id, user_id, type, description, current, add, final)
...
SELECT id, user_id, type, description, current, add, final, created_at, updated_at FROM ledger
WHERE id = $1 
LIMIT 1
`
...
UPDATE ledger
SET user_id = $2, type = $3, description = $4, current = $5, add = $6, final = $7, updated_at = now()
WHERE id = $1
RETURNING id, user_id, type, description, current, add, final, created_at, updated_at
`
